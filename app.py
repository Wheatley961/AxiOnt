...
def main():
    ...
    filter_type = st.selectbox("Выберите тип фильтра", ["Нет", "Класс", "Свойство", "Индивид"])
    ...
    if filter_type != "Нет":
        ...
        filter_value = st.selectbox(f"Выберите {filter_type.lower()}", options)

    html_file = draw_graph(g,
                           filter_type=None if filter_type == "Нет" else filter_type.lower(),
